...
// 加密密码去掉最后8位数
	 ...
	 * @return
	 */
	public static String decodeByXor(String textStr){
		if(textStr == null || textStr.equals("")){
			return "";
		}
		String keyStr = "QdeiOHKDlmdaeMN<>?!@#$~&^(*)+=--678543566{}[]";
		
		StringBuffer retStr = new StringBuffer();
		String[] textArray = textStr.split("-");
		for(int i = 0; i < textArray.length; i += 2){
			int temp = Integer.parseInt(textArray[i], 16);
			int salt = Integer.parseInt(textArray[i+1], 16);
			for(int j = 0; j < keyStr.length(); j++){
				int key = keyStr.charAt(j);
				temp = temp ^ key;
			}
			temp = temp ^ salt;
			retStr.append((char)temp);
		}
